...
from . import _colorama, _asyncio_loop, _defaults, _filters
...
class Logger:
    """
    一个对象，用于将日志消息分派给已配置的处理程序。
    Logger是loguru的核心对象，每个日志配置和使用都会通过调用它的一个方法。只有一个日志记录器，因此在使用之前不需要检索一个。
    一旦导入了' ' logger ' '，就可以使用它来编写关于代码中发生的事件的消息。通过阅读应用程序的输出日志，您可以更好地理解程序的流程，并且更容易跟踪和调试意外行为。
    使用|add|方法添加记录器向其发送日志消息的处理程序。请注意，您可以在导入后立即使用|Logger|，因为它是预先配置的(日志被发送到|sys。Stderr |默认值)。可以用不同的严重级别记录消息，并且可以使用花括号格式化消息(它使用了|str。格式|引擎盖下)。
    当一条消息被记录时，一条“记录”与它相关联。这个记录是一个字典，包含有关日志上下文的信息:时间、函数、文件、行、线程、级别……它还包含模块的' ' __name__ ' '，这就是为什么你不需要命名记录器。
    你不应该自己实例化一个|Logger|，使用' ' from loguru import Logger ' '代替。
    """
    ...
    def add(
        ...
        format : |str| or |callable|_, optional
            在发送到接收器之前用于格式化已记录消息的模板。
        filter : |callable|_, |str| or |dict|, optional
            一个指令，可选地用于决定是否将每条记录的消息发送到接收器。
        colorize : |bool|, optional
            是否应该将格式化消息中包含的颜色标记转换为终端颜色的ansi代码，或者以其他方式剥离。如果None ，则根据接收器是否为tty自动做出选择。
        serialize : |bool|, optional
            日志消息及其记录在发送到接收器之前是否应该首先转换为JSON字符串。
        backtrace : |bool|, optional
            格式化的异常跟踪是否应该向上扩展，超越捕获点，以显示生成错误的完整堆栈跟踪。
        diagnose : |bool|, optional
            异常跟踪是否应该显示变量值以简化调试。
            在生产环境中，应该将其设置为“False”，以避免泄露敏感数据。
        enqueue : |bool|, optional
            要记录的消息在到达接收器之前是否应首先通过多进程安全队列。这在通过多个进程记录到一个文件时非常有用。这还具有使日志记录调用非阻塞的优点。
        catch : |bool|, optional
            接收器处理日志消息时发生的错误是否应该自动捕获。如果' ' True ' '，则在|sys上显示异常消息。但异常不会传播给调用者，从而防止应用程序崩溃。
        **kwargs
            仅在配置协程或文件接收器时有效的其他参数(见下文)。

        当且仅当接收是协程函数时，应用以下参数:
        loop : |AbstractEventLoop|, optional
            异步日志任务将在其中调度和执行的事件循环。如果' ' None ' ' '，则使用的循环是|asyncio返回的循环。Get_running_loop |在日志调用时(如果当前没有正在运行的循环，任务将被丢弃)。

        当且仅当接收是文件路径时，应用以下参数:
        rotation : 控制日志的切割方式，符合条件就会新增一个日志文件，支持的参数如下
            '0:00'
            '1 day'
            '1 week'
            '12:00'
        ...
        retention : 设置日志文件保留的个数，当日志文件超过这个个数会被自动删除
        compression : 日志压缩，支持  zip tar gz tar.gz
        delay : |bool|, optional
    ...
    def open_level_file(
        ...
        """
        开启将不同日志级别的日志输出到不同文件的功能
        :param is_console 是否需要将日志输出到控制台，便于调试
        :param log_dir 日志保存的文件夹
        :param is_info 是否记录info级别的日志
        :param is_success 是否记录success级别的日志
        :param is_warning 是否记录warning级别的日志
        :param is_error 是否记录error级别的日志
        :param is_critical 是否记录critical级别的日志
        :param is_total 是否记录总的日志
        """
        # 移除之前的配置
        self.remove()
        ...
        if is_info:
            self.add(
                os.path.join(log_dir, f"info-{suffix}"),
                level="INFO",
                rotation=rotation,
        ...
        if is_success:
            self.add(
                os.path.join(log_dir, f"success-{suffix}"),
                level="INFO",
                rotation=rotation,
                compression=compression,
                retention=retention,
                filter=lambda x: "SUCCESS" in str(x["level"]).upper(),
            )
        if is_warning:
            self.add(
                os.path.join(log_dir, f"warning-{suffix}"),
                level="INFO",
                rotation=rotation,
                compression=compression,
                retention=retention,
        ...
        if is_error:
            self.add(
                os.path.join(log_dir, f"error-{suffix}"),
                level="INFO",
                rotation=rotation,
                compression=compression,
                retention=retention,
        ...
        if is_critical:
            self.add(
                os.path.join(log_dir, f"critical-{suffix}"),
                level="INFO",
                rotation=rotation,
                compression=compression,
                retention=retention,
        ...
        if is_total:
            self.add(
                os.path.join(log_dir, f"total-{suffix}"),
                level="DEBUG",
                rotation=rotation,
                compression=compression,
                retention=retention,
